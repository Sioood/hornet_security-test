...
export async function getBooksList(
  ...
  try {
    return await fetchBooksFromFakerApi(query, config)
  } catch (error) {
    // oxlint-disable-next-line no-console
    console.warn('[books-source] FakerAPI unavailable, using fixture fallback', error)
    return paginateBooks(getFixtureBooks(), query._page, query._quantity)
  }
}

export async function getBookById(id: number, config: BooksSourceConfig): Promise<Book | null> {
  try {
    return await fetchBookFromFakerApi(id, config)
  } catch (error) {
    // oxlint-disable-next-line no-console
    console.warn(
      `[books-source] FakerAPI unavailable for book ${id}, using fixture fallback`,
      error,
    )
    return getFixtureBooks().find((book) => book.id === id) ?? null
  }
}
